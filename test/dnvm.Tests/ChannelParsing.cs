...
namespace Dnvm.Tests
{
	public class ChannelParsing
	{
		...
		public void FailsToParseInvalidChannels()
		{
			...
			Assert.Throws<DnvmException>(From("1.2.3"));
			Assert.Throws<DnvmException>(From("12.234.234"));
			Assert.Throws<DnvmException>(From("12.234.23xx"));
			Assert.Throws<DnvmException>(From("7.0.0-preview7"));
			Assert.Throws<DnvmException>(From("5.0.3x"));
		}
	}
}